    ...
    return (
      <div className="glass-panel rounded-2xl p-6">
        ...
        <div className="flex flex-col items-center gap-3 py-6">
          <div className="flex h-12 w-12 items-center justify-center rounded-full border border-[#243049] bg-[#111827] text-2xl">
            &#9876;
          </div>
      ...
      <div className="space-y-1.5">
        {players.map((player, i) => {
          ...
          return (
            <div
              key={player.nickname}
              className={`grid grid-cols-[1.5rem_1fr_2.5rem_2.5rem_3.5rem] gap-2 rounded-lg border border-[#243049] px-2 py-2 text-sm transition-colors hover:bg-white/[0.03] ${accentClass}`}
            >
              <span className="text-xs font-bold tabular-nums text-slate-500 self-center">
                {rankLabel}
              </span>
              <span className="self-center truncate font-semibold text-slate-100">
                {player.nickname}
              </span>
